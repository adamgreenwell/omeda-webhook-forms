...
* Description: Captures email addresses and sends them to a webhook endpoint within Omeda
* Version: 1.0
...
class OmedaFormHandler {
    ...
    public function enqueue_scripts() {
		wp_enqueue_script(
			'omeda-form-handler',
			plugins_url('assets/js/form-handler.js', __FILE__),
			array('jquery'),
			$this->version,
			true
		);

        wp_localize_script('omeda-form-handler', 'omedaAjax', array(
            'ajaxurl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('omeda_form_nonce')
        ));
    }
    ...
    public function handle_form_submission() {
        ...
        foreach ($form['fields'] as $field) {
            if (isset($_POST[$field])) {
                $field_name = $this->to_camel_case($field);
                $json_data->$field_name = sanitize_text_field($_POST[$field]);
            }
        }
        ...
        $this->log_error('Preparing webhook request', array(
            'url' => $webhook_url,
            'json_data' => $json_data
        ));
...
$omeda_form_handler = new OmedaFormHandler();